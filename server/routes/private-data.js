...
router.delete('/eventRoles/:id', function (req, res) {
  ...
  pool.connect()
    .then(function (client) {
      client.query('SELECT * FROM roles WHERE event_id IN (SELECT event_id FROM roles WHERE roles.id = $1);',
        [roleId])
        .then(function (result) {
          function deleteRolesAndUserRole(roleId, eventId) {
            pool.connect()
              .then(function (client) {
                client.query('DELETE FROM role_user WHERE role_id=$1;',
                  [roleId])
                  .then(function (result) {
                    pool.connect()
                      .then(function (client) {
                        client.query('DELETE FROM roles WHERE id=$1;',
                          ...
                          .then(function (result) {
                            if (eventId) {
                              pool.connect()
                                .then(function (client) {
                                  client.query('DELETE FROM events WHERE id=$1;',
                                    [eventId])
                                    .then(function (result) {
                                      client.release();
                                      res.sendStatus(200);
          ...
          if (result.rows.length == 1) {
            var eventId = result.rows[0].event_id;
            deleteRolesAndUserRole(roleId, eventId);
          }
          else {
            deleteRolesAndUserRole(roleId);
          }
                        ...
                        pool.connect()
                          .then(function (client) {
                            client.query('INSERT INTO role_user (user_id, role_id) VALUES ($1,$2);',
                              [role.userObject ? role.userObject.id : null, roleId])
                              .then(function (result) {
                                client.release();
                                res.sendStatus(200);
                              })
